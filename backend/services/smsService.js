...
// const CardJourney = require('../models/CardJourney'); // Add this import
// const client = twilio(process.env.TWILIO_ACCOUNT_SID, process.env.TWILIO_AUTH_TOKEN);
// require('dotenv').config();
// // Get user details from database (MISSING FUNCTION)
// // Add this to your smsService.js temporarily for debugging
...
//   try {
//     // Option 1: If you have a separate Users collection
//     // const User = require('../models/User');
//     // const user = await User.findById(userId);
//     // return user;

//     // Option 2: Get user info from CardJourney (more likely for your setup)
//     const cardJourney = await CardJourney.findOne({ customerId: userId });
//     if (!cardJourney) {
//       console.log(`User ${userId} not found`);
//       return null;
//     }

//     // Decrypt mobile number for SMS
//     const { decrypt } = require('../utils/encryption');
//     let mobileNumber = null;

//     try {
//       mobileNumber = decrypt(cardJourney.mobileNumber);
//     } catch (decryptError) {
//       console.error('Failed to decrypt mobile number:', decryptError);
//       return null;
//     }
...
// // Format mobile number with country code (MISSING FUNCTION)
...
//   if (cleanNumber.startsWith('91') && cleanNumber.length === 12) return '+' + cleanNumber;
//   if (cleanNumber.startsWith('0') && cleanNumber.length === 11) return '+91' + cleanNumber.substring(1);
//   if (cleanNumber.length === 10) return '+91' + cleanNumber;

//   // If format is unclear, return as-is and log warning
//   console.warn(`Unusual mobile number format: ${mobile}`);
//   return cleanNumber;
// };
...
//   try {
//     // 1. Get user's mobile number from database
...
//     // 2. Create SMS message content
//     const smsMessage = createSMSMessage(type, data);

//     // 3. Send SMS using Twilio
//     const result = await sendSMS(user.mobileNumber, smsMessage);

//     console.log(`📱 SMS sent to ${user.mobileNumber}: ${smsMessage}`);
...
//       `🏦 Card Update: Your card ${data.cardId} status changed to ${data.newStatus}. Location: ${data.location}`,

//     'CARD_APPROVED': (data) =>
//       `🎉 Hello ${data.customerName}, your card ${data.cardId} has been approved! Expected delivery: ${data.estimatedDelivery}`,

//     'PAYMENT_SUCCESS': (data) => 
//       `💳 Payment Success: ₹${data.amount} paid successfully. Transaction ID: ${data.transactionId}`,

//     'ORDER_SHIPPED': (data) => 
//       `📦 Order Shipped: Your order #${data.orderId} has been shipped. Track: ${data.trackingNumber}`,

//     'ACCOUNT_LOGIN': (data) => 
//       `🔐 Security Alert: Login detected from ${data.location} at ${data.time}. If not you, contact support.`,

//     'OTP_VERIFICATION': (data) => 
//       `🔑 Your OTP is ${data.otp}. Valid for 5 minutes. Do not share with anyone.`,

//     'CARD_DISPATCHED': (data) =>
//       `📮 Your card ${data.cardId} has been dispatched from ${data.location}. Expected delivery in 2-3 business days.`,

//     'CARD_DELIVERED': (data) =>
//       `✅ Great news! Your card ${data.cardId} has been delivered successfully. Enjoy your new card!`,

//     'DELIVERY_FAILED': (data) =>
//       `❌ Delivery attempt failed for card ${data.cardId}. Reason: ${data.failureReason}. We'll retry delivery tomorrow.`
//   };

//   const template = templates[type];
//   if (!template) {
//     console.warn(`No template found for SMS type: ${type}`);
...
//       console.warn(`SMS message is ${message.length} characters (over 160 limit)`);
//     }
...
//     console.log(`✅ SMS sent: ${result.sid} to ${mobileNumber}`);
//     return result;
//   } catch (error) {
...
// services/smsService.js - Fixed version compatible with your project
// services/smsService.js - Robust version that handles your usage pattern
// require('dotenv').config();
// const twilio = require('twilio');
...
//       if (cardJourney.mobileNumber && this.isEncrypted(cardJourney.mobileNumber)) {
//         try {
//           const { decrypt } = require('../utils/encryption');
//           mobileNumber = decrypt(cardJourney.mobileNumber);
//         } catch (decryptError) {
...
//       return {
//         id: cardJourney.customerId || cardJourney.cardId,
//         name: cardJourney.customerName,
...
//   isEncrypted(value) {
//     // Simple check - encrypted values are usually longer and contain special chars
...
//     const cleanNumber = mobile.replace(/[^\d+]/g, '');
//     console.log("cleanNumber  came till here", cleanNumber);
...
//       console.log(`🔍 User found: ${user ? user.name : 'Unknown'} (${userId})`);
//       if (!user || !user.mobileNumber) {
...
//     const template = templates[type];
//     if (!template) {
//       console.warn(`⚠️ No template found for SMS type: ${type}`);
//       return `Update: Your card status has been updated. Card ID: ${data.cardId || 'N/A'}`;
//     }
